...
def render_schedule_options(players: List[Player]) -> ScheduleConfig:
    ...
    minimum_players = st.number_input("Number Players", min_value=Schedule.min_players, max_value=Schedule.max_players, value=st.session_state.get("min_players", Schedule.max_players), key="min_players", disabled=automatic_player_counts)

    if automatic_player_counts: 
        st.session_state.min_females = 3 if st.session_state.min_players > 9 else 2
    minimum_females = st.number_input("Minnimum Females", min_value=0, max_value=Schedule.max_players, value = st.session_state.get("min_females", 3), key="min_females", disabled=automatic_player_counts)
    st.divider()
